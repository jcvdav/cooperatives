...
jones_cheung_genus <- jones_cheung %>% 
  mutate(genus = str_extract(string = Complete_Name, pattern = "([^\\s]+)")) %>% 
  group_by(genus) %>% 
  summarize_if(is.numeric, mean) %>% 
  select(genus, genusV = VulnerabilityIndex, genusR = RiskOfImpact)
```

### Read species scientific names

```{r}
vulnerability <- read.csv(here("raw_data", "spp_sci_name.csv"))  %>% 
  mutate(genus = str_extract(string = Complete_Name, pattern = "([^\\s]+)")) %>% 
  ...
  select(Original_Order, Vulnerability) %>% 
  janitor::clean_names()
```
...
coop <- read.csv(here("raw_data", "CoopResilience.csv"),
  ...
  left_join(vulnerability, by = "original_order") %>% 
  filter(!fish_type %in% c("freshwater", "freshwater & diadromous")) %>% 
  dplyr::select(original_order, fishery_id, host_country, target_species, lon, lat, human_development_index, vessels_coop, processed, codified_penalties, cooperative_administration, direct_enforcement, financial_services, information_sharing, percent_of_gdp_from_fishing, short_participants, short_catch_use_numeric, sea_level_vulnerability, sea_temp_vulnerability, ocean_acidification_vulnerability, ozone_uv_vulnerability, recovery_time, temporal_no_take, spatial_no_take, msc_certification, recorded_fishery_closure, poverty_index, institutional_stability, corruption, itq, ivq, ieq, iq, turf, coop, mpa, profit_sharing, coharvest, cooperative_gear_restrictions, cooperative_size_limit, gear_sharing, habitat_restoration, restocking, research_support, temperature_change, number_of_species, vulnerability) %>% 
  mutate(sea_level_vulnerability = sea_level_vulnerability,
         ozone_uv_vulnerability = ozone_uv_vulnerability,
         sea_temp_vulnerability = sea_temp_vulnerability,
...
coop_clean <- cbind(coop_text, coop_numbers) %>%
  ...
  mutate(ecological_exposure = temperature_change,
         ecological_recovery_potential = (recovery_time +
                                            mpa) /2,
