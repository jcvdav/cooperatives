...
coop_clean %>% 
  dplyr::select(lon, lat, social_adaptive_capacity, social_sensitivity, ecological_sensitivity, ecological_exposure, ecological_recovery_potential, score) %>% 
...
Cambiar Puerto Oscuro a -31.427525,-71.6063907
...
nas <- numeric(length = 20)
for (i in 1:20){
  testing <- pca_data[,-i]
  nas[i] <- 93 - sum(apply(testing, 1, function(x){any(is.na(x))}))
}
